...
void task_controle_PI(void *pvParameters)
{
    while (1)
    {
        float rpm = wheel_GetRPM(); //le vel atual
        float erro = ref_rpm - rpm; //calcula erro
		...
		//Compara variavel para nao deixar saturar
		...
		//Garante duty minimo para nao desligar em baixa vel
        if (ref_rpm > 0.0f && duty > 0.0f && duty < DUTY_MIN_MOVIMENTO)
        {
            duty = DUTY_MIN_MOVIMENTO;
        }
		
		//atualiza integral do erro
        if (ref_rpm > 0.0f && duty_unsat < DUTY_MAX)
        {
            erro_integral += erro * TS_CONTROLE_S; //integral discreta
        }
...
void hnd_post_vel(
    ...
    coap_pdu_t *response)
{
	//variaveis para receber payload COAP
    size_t size;
    const uint8_t *data;

    if (coap_get_data(request, &size, &data)) //extrai payload
    {
        char buffer[20]; 	//buffer temporario para converter bytes em string

        if (size >= sizeof(buffer))	
            size = sizeof(buffer) - 1;	//memoria nao estora

        memcpy(buffer, data, size);
        buffer[size] = '\0';		    //copia bytes e converte para string

        ref_rpm = atof(buffer);   //converte texto para float

        if (ref_rpm < 0.0f)	//ref negativa
            ref_rpm = 0.0f;	

        if (ref_rpm == 0.0f) //ref = 0
        {
            erro_integral = 0.0f;
            duty_atual = 0.0f;
            motor_Stop();
	...
	//Define que resposta foi alterada
    coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED); 

    const char *resp_str = "OK"; //resposta do COAP
	
	//coloca o 'ok' no payload da resposta pro python
    coap_add_data(
        response,
        strlen(resp_str),
...
//Resposta com conteudo valido
    coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);

//Envia string para python
    coap_add_data(
        response,
        strlen(resp),
        (const uint8_t *)resp);
...
{
    coap_startup(); //incializa biblioteca

    coap_context_t *ctx = coap_new_context(NULL); //cria contexto
	...
	//config do endereco do servidor
	...
	//COAP fica rodando a cada 10ms
	while (1)
	{
	    coap_io_process(ctx, 50); //tempo maximo de espera dentro da funcao
	    vTaskDelay(pdMS_TO_TICKS(10)); //depois espera mais 10ms
	}
}
